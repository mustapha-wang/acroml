...
(function() {
  ...
  function getModuleSync(resolvedPath) {
    ...
    if (req.status >= 200 && req.status < 400) {
      ...
      if (resolvedUri.substr(resolvedUri.length-4).toLowerCase()=='.jsx'){//add by wxh
  ...
  function evaluateModule(__dirname, __filename, __parentModule__, __moduleScriptText__) {
    ...
    //console.log(__filename);//, module,__moduleScriptText__);//add by wxh
  ...
  let importMap=$('script[type="importmap"]').text();//add by wxh
  //console.log(importMap);
  importMap=JSON.parse(importMap).imports;//add by wxh
  ...
  function __Require__(__dirname, __filename, __parentModule__) {
    ...
    require.resolve = function(path) {
      //console.log(path);
      if (importMap[path]){//add by wxh
        path=importMap[path];//add by wxh
      }
    ...
    require.deleteCache=function(path){//add by wxh,2023/8/23
      var resolvedPath = require.resolve(path);
